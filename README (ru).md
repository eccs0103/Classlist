...
## Guide
Для работы с программой потребуется расписание. Расписание должен быть собран в виде файла JSON соответственно следующей схеме:
```typescript
interface Lesson { // Структура одиночных уроков, таких, какие проводят в школе
	title: String, // Название урока
	description: String, // Описание урока
	begin: Number, // Начало (в миллисекундах)
	duration: Number, // Длительность (в миллисекундах)
}

interface Pair { // Структура парных уроков, таких, какие проводят в университете
	title: String, // Название урока
	description: String, // Описание урока
	begin: Number, // Начало (в миллисекундах)
	duration: Number, // Длительность (в миллисекундах)
	recess: Number, // Длительность перемены среди пар (в миллисекундах)
}

interface Weekday { // Структура дня, как будних так и выходных
	title: String, // Название дня
	subjects: Array<Lesson | Pair>, // Уроки в течении дня
}

interface Workweek { // Структура учебного периода. В основном это неделя, хотя может и отличаться.
	weekdays: Array<Weekday>, // Дни в течении периода
}
```
Один из простейших примеров JSON структуры ниже:
```json
{
	"weekdays": [
		{
...
```
В качестве алтернативы можете использовать внутренний движок программы, чтобы через консоль, быстро создать из вашего расписания файл JSON.
```javascript
...
Workweek.export(workweek); // Нужные вам данные
```
## Feed
### Update 1.0.0
