...
void async function () {
	try {
		...
		buttonDownloadSchedule.addEventListener(`click`, (event) => {
			...
			if (schedule !== null) {
				navigator.download(new File([JSON.stringify(schedule.notation)], schedule.title));
			}
		});

		buttonRemoveSchedule.addEventListener(`click`, async (event) => {
			if (await window.confirmAsync(`Schedule will be removed from memory forever. Are you sure?`, `Warning`)) {
